...
namespace LearnLink.Testing
{
    ...
    public class GradeServiceTests
    {
        ...
        private static GradeFilterModel Filter(string sortBy = "date", bool descending = true,
            int page = 1, int size = 50, string student = "", string subject = "")
            => new GradeFilterModel
            {
                SortBy = sortBy,
        ...
        {
            var page = await service.GetFilteredGradesAsync(Filter(student: "Ivan Petrov"));
            var total = await service.GetTotalFilteredGradesAsync(Filter(student: "Ivan Petrov"));
        ...
        {
            var page = await service.GetFilteredGradesAsync(Filter(subject: "History"));
            var total = await service.GetTotalFilteredGradesAsync(Filter(subject: "History"));
        ...
        public async Task GetFilteredGradesAsync_ReturnsNewestFirstByDefault()
        {
            ...
            var page = await service.GetFilteredGradesAsync(Filter());
        ...
        {
            var first = await service.GetFilteredGradesAsync(Filter(page: 1, size: 2));
            var second = await service.GetFilteredGradesAsync(Filter(page: 2, size: 2));
        ...
        {
            var ascending = await service.GetFilteredGradesAsync(Filter(sortBy: "student", descending: false));
            var descending = await service.GetFilteredGradesAsync(Filter(sortBy: "student"));
        ...
        {
            var page = await service.GetFilteredGradesAsync(Filter(sortBy: "value", descending: false));
        ...
        {
            var unknown = await service.GetFilteredGradesAsync(Filter(sortBy: "banana"));
            var byDate = await service.GetFilteredGradesAsync(Filter());
